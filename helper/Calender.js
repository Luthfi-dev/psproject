function Calender(){
  ...
  const monthFormat = 'm';
  const longMonthList = ['January','February','March','April','May','June','July','August','September','October','November','December']

  const shortMonthList = ['Jan','Feb','Marc','Apr','May','Jun','Jul','Aug','Sept','Oct','Nov','Dec']

  this.diffDays = (date, curDate = null ) =>{
    const currentDate = curDate === null ? new Date() : new Date(curDate)
    const oldDate = new Date(date)
    const differenceInTime = currentDate.getTime() - oldDate.getTime()
    const differenceInDays = differenceInTime / (1000 * 3600 * 24)
    return Math.ceil(differenceInDays)
  }
  this.month = (monthNumber,monthFormat)=>{
    monthFormat=monthFormat;
    if(monthFormat==='M'){
      return longMonthList[monthNumber-1];
    }else{
      return shortMonthList[monthNumber-1];
    }
  }

  this.dateFormat = (date, mask, utc, gmt)=>{
    if (
      arguments.length === 1 &&
      typeof date === "string" &&
      !/\d/.test(date)
