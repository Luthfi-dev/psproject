...
const UserInfo = ({appsData})=>{
  const session = appsData.session;
  const fullName = session.fullName;
  const firstName = fullName.split(" ")[0];
  const avatar = session.avatar ? session.avatar:'media/avatars/150-20.jpg';
    ...
    <div className="aside-toolbar flex-column-auto" id="kt_aside_toolbar">
      <div className="aside-user d-flex align-items-sm-center justify-content-center py-5">
        <div className='symbol symbol-50px'>
          <div className=" container-img w-50px">
            <Image
            src={avatar+'?subfolder=avatars&'}
            layout="fill"
            className='img symbol symbol-50px'
            priority={true}
            loader={session.avatar ? serverAssets : imgLoader}
            />
          </div>
        </div>
        <div className="aside-user-info flex-row-fluid flex-wrap ms-5">
          <div className="d-flex">
            <div className="flex-grow-1 me-2">
              <a href="#" className="text-white text-hover-primary fs-6 fw-bold">{firstName}</a>
              <span className="text-gray-600 fw-bold d-block fs-8 mb-1">{session.username}</span>
